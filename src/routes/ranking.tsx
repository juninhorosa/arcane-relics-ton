...
function Ranking() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Ranking de Nações (usando o cache/snapshot mais recente)
      ...
      if (nRanking && nRanking.length > 0) {
        ...
        // Fallback para agregação em tempo real se o snapshot do dia não existir
  ...
  return (
    <div className="p-4 bg-slate-900 min-h-screen text-white pb-20">
      ...
      {loading ? (
        <div className="text-center py-10 italic text-slate-500">Consultando oráculos...</div>
      ) : tab === 'players' ? (
            ...
            <div key={i} className={`flex items-center p-3 rounded-lg border ${i === 0 ? 'bg-yellow-900/20 border-yellow-600' : 'bg-slate-800/50 border-slate-700'}`}>
              ...
              <div className="flex-1">
                ...
                <div className="text-[10px] text-slate-400 uppercase tracking-tighter">
                  {p.nations?.name} • LVL {p.level}
                </div>
              </div>
              <div className="text-right">
                <div className="text-yellow-500 font-bold">💰 {p.gold.toLocaleString()}</div>
                <div className="text-[10px] text-green-500">{p.wins} Vitórias</div>
              </div>
            </div>
          ))}
        </div>
